...
from inn_model import feature_extractor
        ...
        if finetune_use_gt:
            ...
            if not xT_cov_loss:
                with th.no_grad():
                    ...
                    if xT_std_align:
                        noise_gt_pred = predicted_xT_from_gt-z_ys
            ...
            if not loss_in_image_space: 
                ...
                b1, x_start_details = feature_extractor(z_start_gt)
                #model_output_pedict_gt_rgb = self.decode_first_stage(model_output_pedict_gt, first_stage_model, no_grad=False)
                b2, model_output_pedict_gt_rgb_details = feature_extractor(model_output_pedict_gt)
                terms["_details"] = 0.2*mean_flat((x_start_details - model_output_pedict_gt_rgb_details) ** 2)
                terms["loss"] += (terms["_details"]*finetune_use_gt)
            else:
