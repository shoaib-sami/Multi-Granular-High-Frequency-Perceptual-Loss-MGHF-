...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
            ...
            type=str,
            default="HF-Diff",
            choices=["HF-Diff","SinSR",'realsrx4', 'bicsrx4_opencv', 'bicsrx4_matlab'],
            help="Chopping forward.",
            )
...
def get_configs(args):
    if args.config is None:
        if args.task == "HF-Diff":
            configs = OmegaConf.load('./configs/HF-Diff.yaml')
    ...
    if args.ckpt is None:
        if not ckpt_dir.exists():
            ckpt_dir.mkdir()
        if args.task == "SinSR":
            ckpt_path = ckpt_dir / f'HF-Diff.pth'
        elif args.task == 'realsrx4':
    ...
    if not ckpt_path.exists():
        if args.task == "HF-Diff":
            load_file_from_url(
                url=f"https://drive.google.com/file/d/12sRBlIObbrmskInfx7bvExd76puXbUhx/view?usp=drive_link",
                model_dir=ckpt_dir,
                progress=True,
